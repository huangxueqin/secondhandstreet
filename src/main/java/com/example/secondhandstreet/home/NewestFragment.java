...
public class NewestFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener{
    private final String TAG = "HXQ_TAG";
    ...
    private boolean mIsJustInitialized = true;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mRecListAdapter = new SimpleItemInfoCardsAdapter(getActivity(), CacheManager.getInstance().getNewestCache());
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_home_newest, container, false);
    ...
    private void refreshData() {
        ...
        if(!mIsLoadingData) {
            ...
            mRefreshTask.execute(Settings.NEWEST_URL);
        }
    }
    ...
    private void loadMoreData() {
        ...
        if(!mIsLoadingData && mCurrentPage < mTotalPage-1) {
            ...
            String url = Settings.NEWEST_URL + "?page=" + (mCurrentPage+2);
    ...
    private class SimpleOnScrollListener extends RecyclerView.OnScrollListener{
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            Log.d(TAG, "totalCount = " + totalCount + ", visibleCount = " + visibleCount);
    ...
    private class GetProductListTask extends AsyncTask<String, Void, List<ItemInfo>> {
        ...
        protected void onPostExecute(List<ItemInfo> itemInfos) {
            ...
            if(itemInfos == null) {
                if(mRequestType == REQUEST_TYPE_REFRESH) {
                    Toast.makeText(NewestFragment.this.getActivity(), "刷新最新发布商品失败", Toast.LENGTH_SHORT).show();
                    mRefreshIndicator.setRefreshing(false);
                }
                else {
                    Toast.makeText(NewestFragment.this.getActivity(), "加载最新发布商品失败", Toast.LENGTH_SHORT).show();
                }
            }
            else {
                if (mRequestType == REQUEST_TYPE_REFRESH) {
                ...
                if(mNeedWriteCache) {
                    ...
                    CacheManager.getInstance().writeNewestCache(itemInfos);
                }
            }
            mRequestType = REQUEST_TYPE_NONE;
        ...
        protected List<ItemInfo> doInBackground(String... params) {
            ...
            if (response != null && !response.has(Settings.JSON_KEY_ERROR)) {
                LogUtil.logd("newest item is: " + response.toString());
